...
static void hid_mouse_usb_close(void);
static void hid_mouse_usb_open(void);
...
#define KEYBD_REPORT_DESC_PATH	"/sys/kernel/config/usb_gadget/hid_kb/functions/hid.usb1/report_desc"
...
static void hid_device_usb_init(void)
{
	...
	safe_system("mount -t configfs none /sys/kernel/config &> /dev/null");
    ...
    if(status) {
    	...
    	safe_system("echo American Megatrends Inc. > /sys/kernel/config/usb_gadget/hid/strings/0x409/manufacturer");
    	safe_system("echo Virtual Mouse Gadget > /sys/kernel/config/usb_gadget/hid/strings/0x409/product");
    	...
    	if(MouseMode == ABSOLUTE_MOUSE_MODE) {
			...
			safe_system("echo 4 > /sys/kernel/config/usb_gadget/hid/functions/hid.usb0/report_length");
    	...
    	//keyboard 
    	safe_system("mkdir /sys/kernel/config/usb_gadget/hid_kb");
    	safe_system("echo 0x046b >/sys/kernel/config/usb_gadget/hid_kb/idVendor");
    	safe_system("echo 0xFF10 >/sys/kernel/config/usb_gadget/hid_kb/idProduct");
    	safe_system("mkdir /sys/kernel/config/usb_gadget/hid_kb/strings/0x409");
    	safe_system("echo 0123456789 > /sys/kernel/config/usb_gadget/hid_kb/strings/0x409/serialnumber");
    	safe_system("echo American Megatrends Inc. > /sys/kernel/config/usb_gadget/hid_kb/strings/0x409/manufacturer");
    	safe_system("echo Virtual Keyboard Gadget > /sys/kernel/config/usb_gadget/hid_kb/strings/0x409/product");
    	safe_system("mkdir /sys/kernel/config/usb_gadget/hid_kb/configs/c.1");
    	safe_system("mkdir /sys/kernel/config/usb_gadget/hid_kb/configs/c.1/strings/0x409");
    	safe_system("echo config 1 > /sys/kernel/config/usb_gadget/hid_kb/configs/c.1/strings/0x409/configuration");
		
		//keyboard usb 1
    	safe_system("mkdir /sys/kernel/config/usb_gadget/hid_kb/functions/hid.usb1");

		//protocol 2:mouse 1:keyboard
    	safe_system("echo 1 > /sys/kernel/config/usb_gadget/hid_kb/functions/hid.usb1/protocol");
		//sub clasee 1:Boot Interface 
    	safe_system("echo 1 > /sys/kernel/config/usb_gadget/hid_kb/functions/hid.usb1/subclass");	   

		//report length 	
    	safe_system("echo 8 > /sys/kernel/config/usb_gadget/hid_kb/functions/hid.usb1/report_length");
    	
		//report descriptor
    	SetKeyboardReportDesc();

    	safe_system("cd /sys/kernel/config/usb_gadget/hid_kb && ln -s functions/hid.usb1 configs/c.1/");
    	safe_system("echo 0xE0 > /sys/kernel/config/usb_gadget/hid_kb/configs/c.1/bmAttributes");
    }
}
...
{
	safe_system("echo > /sys/kernel/config/usb_gadget/hid/UDC");
	safe_system("echo > /sys/kernel/config/usb_gadget/hid_kb/UDC");
}

static void hid_device_usb_open(void)
{
	safe_system("echo 1e6a0000.usb-vhub:p4 > /sys/kernel/config/usb_gadget/hid/UDC");
	safe_system("echo 1e6a0000.usb-vhub:p6 > /sys/kernel/config/usb_gadget/hid_kb/UDC");
...
int unload_hid_resource(char *reg_process)
{
	...
	if(MAX_SUPPORT_PROCESS <= resource ) {
		printf("Maxmium process not support %s\n",reg_process);
		return FAIL;
	}
	fp = fopen(HID_RESOURCE_FILE,"w+");
...
/**
 *@fn IsAlpha
 *@brief Helps to findout the presence of alphabetic character in string
 *@param instr - Input string
 *@return Returns -1 on failue
 *            Returns 0 on success
*/
int IsAlpha(char *instr)
{
    int i = 0;
...
static int IsProcRunning( char *procarg)
{
    ...
    do
    {
		...
		/* Reason for false positive - use to check if folder exist, never write/update after stat. */
		/* coverity[Security best practices violations : FALSE] */
