...
uint32 apml_init_fdk(APML_DEV_CTL *ctl,int BMCInst)
{
	uint32	i, j, res;
	unsigned long reg;
    struct stat hostcomplete;
    int hostcomplete_retry=10, mount_retry=3, init_retry=3;
    ...
    while (hostcomplete_retry-- > 0 && stat("/var/hostcomplete", &hostcomplete) != 0)
    {
        sleep(3);
        printf("APML waits for host complete, retry %d...\n", 10-hostcomplete_retry);
    }

    if (hostcomplete_retry < 0)
    {
        printf("APML waits for host complete time out\n");
        return -1;
    } 
    ...
    while (init_retry-- > 0)
    {
        ...
        {
            printf("Initialize APML user platform retry %d...\n", 3-init_retry);
            sleep(1);
        }
    }
	...
	int			BMCInst)
{
	...
	*data = 0;
	...
	dev->stat.bytes_rcvd += 1;  /* 1 byte data */
	...
	int			BMCInst)
{
	...
	//SB-RMI Read Processor Register Command for Write Protocol
	res = apml_block_process_call(ctl, socketid, proc_addr, cmd.xlen, wr_data, 0, in_data,BMCInst); 
	if (res != APML_SUCCESS)
	{
	...
	//SB-RMI Read Processor Register Command for Reload Read Address and Read Data/Status Protocol
	res = apml_block_process_call(ctl, socketid, proc_addr, 1, wr_data, cmd.rlen, in_data,BMCInst); 
	if (res != APML_SUCCESS)
	{
		apml_hal_err_log(ctl, socketid, cmd, proc_addr, 0, res);
		return(res);
	...
	int			BMCInst)
{
	...
	res = apml_load_sar(ctl, LOAD_ADDRESS_COMMAND, proc_addr, proc_ndx, core_ndx, 0, reg_id,BMCInst);
	...
	wr_data[0] = 0x71;
	wr_data[1] = reg_val_size + 2;  //APML 2.0, this value == reg_val_size + 2
	wr_data[2] = 0x87;
	...
	//SB-RMI Write Processor Register Command for WRite Protocol
	res = apml_block_process_call(ctl, proc_ndx, proc_addr, cmd.xlen, wr_data, 0, in_data,BMCInst);
	if (res != APML_SUCCESS)
	{
	...
	//SB-RMI Write Processor Register COmmand for Reload Read Address and Read Status Protocol
	res = apml_block_process_call(ctl, proc_ndx, proc_addr, 1, wr_data, cmd.rlen, in_data,BMCInst);
	if (res != APML_SUCCESS)
	{
		apml_hal_err_log(ctl, proc_ndx, cmd, proc_addr, core_ndx, res);
		return(res);
	...
	int			BMCInst)
{
	...
	if (rd_len == 0) rd_len++;  //for write protocol
	if (res != APML_SUCCESS)
		return(res);
	...
	int			BMCInst)
{
	...
	//SB-RMI Load Address Command for Write Protocol
	res = apml_block_process_call(ctl, proc_ndx, proc_addr, cmd.xlen, wr_data, 0, in_data,BMCInst); 
	if (res != APML_SUCCESS)
	{
		apml_hal_err_log(ctl, proc_ndx, cmd, proc_addr, core_ndx, res);
	...
	//SB-RMI Load Address Command for Reload Read Address and Read Status Protocol
	res = apml_block_process_call(ctl, proc_ndx, proc_addr, 1, wr_data, cmd.rlen, in_data,BMCInst);	
	if (res != APML_SUCCESS)
	{
		apml_hal_err_log(ctl, proc_ndx, cmd, proc_addr, core_ndx, res);
