...
typedef struct apml_ctl_struct {
	...
	uint8			rmi_thread_target[APML_MAX_PROCS];
	uint8			threads[APML_MAX_PROCS];
...
extern uint32 apml_read_mailbox_service_with_mboxcmd(
    APML_DEV_CTL	*dev,
    uint8       socketid,
	uint8		proc_addr,
	uint8		mboxcmd,
...
extern uint32 apml_write_mailbox_service_with_mboxcmd(
        APML_DEV_CTL    *dev,
        uint8           socketid,
        uint8           proc_addr,
    	uint8		    mboxcmd,
...
extern uint32 apml_writeread_mailbox_service_with_mboxcmd(
        APML_DEV_CTL    *dev,
        uint8       socketid,
        uint8       proc_addr,
    	uint8		mboxcmd,
