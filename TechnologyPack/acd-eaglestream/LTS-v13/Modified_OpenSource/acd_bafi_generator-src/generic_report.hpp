...
class GenericReport
{
        ...
        for (auto const& [socketId, socketTscData] : summary.tsc)
        {
                ...
                socketTscData.pcu_first_rmca_tsc_cfg)
            {
                ...
                if (summary.cpuType == "SPR")
                {
                    ...
                    ss3 << "pcu_first_rmca_tsc_cfg: " <<
                        int_to_hex(socketTscData.pcu_first_rmca_tsc_cfg);
                ...
                ss4 << "IERR occurred: " << std::boolalpha
                    << socketTscData.ierr_on_socket;
                json entry4 = ss4.str();
                tscData.push_back(entry4);

                std::stringstream ss5;
                ss5 << "MCERR occurred: " << std::boolalpha
                ...
                if (summary.cpuType == "SPR")
                {
                    ...
                    ss6 << "RMCA occurred: " << std::boolalpha
                        << socketTscData.rmca_on_socket;
                    json entry6 = ss6.str();
  ...
  protected:
            ...
            json& summaryEntry)
    {
        ...
        {
            if (ctx.mcerrErr.rmca_internal)
            {
                ss << "IntRMCA, ";
            }
            if (ctx.mcerrErr.rmsmi_internal)
            {
                ss << "IntRMSMI, ";
            }
        ...
        if (Cpu == "SPR")
        {
            if (firstRmcaSrc && (ctx.rMcerrErr.firstMcerrValid))
            {
                ...
                {
                    ss << "RMCA_FirstMCERR = " << *firstRmcaSrc << ", RMCA_FirstMCERR value"
                        " doesn't match any MCE bank#, manual review may be required";
                }
            }
